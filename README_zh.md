# 📖 智驾时代：Agent进化简史

> 从Prompt到自进化的AI驾驶之路
> 作者：**Yason** · MindApex · VokoForge

[![Stars](https://img.shields.io/github/stars/Madapexai/agent-evolution-book?style=social)](https://github.com/Madapexai/agent-evolution-book)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Language](https://img.shields.io/badge/lang-中文%20%7C%20English-blue)]()
[![Chapters](https://img.shields.io/badge/chapters-18%20%2B%203%20appendices-orange)]()

---

## 简介

一辆从手摇启动到自动驾驶的进化之路。本书用自动驾驶的隐喻，将Agent从Prompt工程到Loop自进化的完整历程娓娓道来。每一章都是一个驾驶场景，让你在理解Agent的同时，也看到AI的"驾驶"技术如何一步步走向成熟。

这本书不仅告诉你Agent是什么，更展示它们如何进化——从手摇启动的Prompt，到装上底盘的Harness系统，再到即将定义下一个软件工程时代的自驱动Loop。

> 📖 [English README](README.md)

## 章节

| 编号 | 标题 | 文件 |
|------|------|------|
| 00 | 序章 未来已来 | [zh/00_prologue.md](zh/00_prologue.md) |
| 01 | 第1章 Agent是什么：一辆会思考的自动驾驶汽车 | [zh/01.md](zh/01.md) |
| 02 | 第2章 手摇启动时代：Prompt工程 | [zh/02.md](zh/02.md) |
| 03 | 第3章 视野开启：Context工程 | [zh/03.md](zh/03.md) |
| 04 | 第4章 装上底盘：Harness时代 | [zh/04.md](zh/04.md) |
| 05 | 第5章 大脑：大模型是怎么思考的 | [zh/05.md](zh/05.md) |
| 06 | 第6章 记忆系统：Agent的"海马体" | [zh/06.md](zh/06.md) |
| 07 | 第7章 上下文工程：Agent的"视野" | [zh/07.md](zh/07.md) |
| 08 | 第8章 工具与插件：Agent的"手和脚" | [zh/08.md](zh/08.md) |
| 09 | 第9章 Harness深度解析：七层底盘架构 | [zh/09.md](zh/09.md) |
| 10 | 第10章 子Agent团队：从单车到车队 | [zh/10.md](zh/10.md) |
| 11 | 第11章 可观测与可审计：透明驾驶舱 | [zh/11.md](zh/11.md) |
| 12 | 第12章 实战一：AI网页设计师 | [zh/12.md](zh/12.md) |
| 13 | 第13章 实战二：内容运营与数据Agent | [zh/13.md](zh/13.md) |
| 14 | 第14章 实战三：自动化测试Agent | [zh/14.md](zh/14.md) |
| 15 | 第15章 实战四：Agent Mesh智能团队 | [zh/15.md](zh/15.md) |
| 16 | 第16章 Loop革命：自驱动循环 | [zh/16.md](zh/16.md) |
| 17 | 第17章 自进化：Agent的成长之路 | [zh/17.md](zh/17.md) |
| 18 | 第18章 你就是副驾：智驾时代的人类角色 | [zh/18.md](zh/18.md) |
| A | 附录A：Agent工具清单 | [zh/appendix_a.md](zh/appendix_a.md) |
| B | 附录B：学习路线图 | [zh/appendix_b.md](zh/appendix_b.md) |
| C | 附录C：术语表 | [zh/appendix_c.md](zh/appendix_c.md) |

## 链接

- 🌐 博客：[blogs.yason.click](https://blogs.yason.click/)
- 🏢 官网：[madapexai.com](https://www.madapexai.com/)
- 📚 飞书知识库：[智驾时代：Agent进化简史](https://mindapex.feishu.cn/wiki/E7JmwiqTLiAiThkWNPccAS3DnRe)

## 同系列书籍

- 🤖 [给AI当老板：Agent作战团队](https://github.com/Madapexai/ai-agent-team-book) — 21章智能体团队实战手册
- 🔄 [别调Prompt了，去写Loop](https://github.com/Madapexai/loop-engineering-book) — AI Agent时代的软件工程新范式

## 作者

**Yason** · 非典型程序员 / Agent 团队架构师

过去，我通过管理团队来创造价值；如今，AI 正在重塑一切。管理的本质正在从「管人」转向「管 Agent」——以 Agent 为兵，以系统为阵，打造一支真正有战斗力的超级团队。

- 博客：[blogs.yason.click](https://blogs.yason.click/)
- 官网：[madapexai.com](https://www.madapexai.com/)

## 许可证

[MIT License](LICENSE) — 自由使用、复制、修改和分发。

---

*MindApex · VokoForge 出品*
